...
const handleReserve = (e, idev) => {
  console.log('here')
  fetch(
    `http://localhost:5000/api/event/reserver/ev/`,
    {
      body: {
        eventId: idev,
        nbePlaces: nbr
    ...
    <>
      <Card sx={{ maxWidth: 345 }}>
        ...
        <p style={{ marginLeft: 10, fontWeight: "bold" }}>
          Début: {moment(item?.DateDebut).format("MMMM Do YYYY")}
        </p>

        <p style={{ marginLeft: 10, fontWeight: "bold" }}>
          Fin: {moment(item?.DateFin).format("MMMM Do YYYY")}
        </p>
        <img
