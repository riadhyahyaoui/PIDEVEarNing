...
module.exports.getAllevenements = async (req, res) => {
  ...
  const user = await userModel.find({ _id: "62743c743ef9c83b00f33acf" }).lean();
  let checkIfLiked = (element) => element == "62743c743ef9c83b00f33acf";
  let eventList = [];
  evenements.map((ev) => {
    let obj = ev;
  ...
  //   if (decodedToken && decodedToken.sub) {
   await evenementModel
    .findByIdAndUpdate(req.params.evenementModelId, {
      $addToSet: { likes: "62743c743ef9c83b00f33acf" },
    })
    .lean();
    const evenements = await evenementModel.find().lean();
  const user = await userModel.find({ _id: "62743c743ef9c83b00f33acf" }).lean();
...
module.exports.dislikeevenementModel = async (req, res) => {
  ...
  await evenementModel
    .findByIdAndUpdate(req.params.evenementModelId, {
      $pull: { likes: "62743c743ef9c83b00f33acf" },
    })
    .lean();
  await evenementModel
    .findByIdAndUpdate(req.params.evenementModelId, {
      $addToSet: { dislikes: "62743c743ef9c83b00f33acf" },
    })
    .lean();
    const evenements = await evenementModel.find().lean();
  const user = await userModel.find({ _id: "62743c743ef9c83b00f33acf" }).lean();
...
module.exports.reservePlace = async (req, res) => {
  ...
  let us = await userModel
    .findOne({ _id: "62743c743ef9c83b00f33acf" /*decodedToken.sub*/ })
    .lean();
